...
xm = 1
alp = [1.2 + 0.1*i for i in range(1, 101)]
c = 1
dt = 5
...
def qiu_z(u, zd, zu, m, z):
    for i in range(100):
        ...
        for j in range(len(a)):
            if a[j] < 1:
                continue
            if xm + xm / (alp[i] * a[j] - 1) + u * c * a[j] < M:
                M = xm + xm / (alp[i] * a[j] - 1) + u * c * a[j]
                x1 = j
        z[i] = a[x1]
    print(z)
...
def bm_sf_u(m):
    ...
    Epsilon = 0.00005
    d = 1
    z = 1 / np.sqrt(np.array(alp)*u*c) + 1/np.array(alp)
...
def dw_f_la(m):
    Ex = np.array(xm + xm / (np.array(alp) - 1))
    global mt, x2, Max
    x2 = 0
    Max = 0
    R = np.ones(100)
    for j in range(1, m-100+1):
        for i in range(100):
            Ex1 = xm + xm / ((alp[i] * (R[i] + 1)) - 1)
            dif = Ex[i] - Ex1
            if dif > Max:
                x2 = i
                Max = dif
        Max = 0
        Ex[x2] = xm + xm / ((alp[x2]*(R[x2]+1))-1)
        R[x2] += 1
        mt = np.sum(Ex)
    return R
...
if __name__ == '__main__':
    ...
    ax4.set_title("Fig.h4. Cost and delay trade-off", fontsize=15)
    y4 = np.zeros(101)
